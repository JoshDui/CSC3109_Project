#!/usr/bin/env python3
"""Evaluate Semantic-Guided CG-AF CNN checkpoints with emulated quantized weights.

This tool is a numerical/deployment-proxy evaluation, not a backend export or
accelerated NPU benchmark.  Quantized Conv2d/Linear replacements store weights
as integer buffers, dequantize those weights back to floating point for
``torch.nn.functional.conv2d``/``linear``, and apply calibrated activation Q/DQ
in PyTorch before each wrapped module.

Checkpoint loading is intended for trusted project-generated checkpoints only.
The project ``.pt`` files may include Python metadata dicts/lists alongside
tensors, so this script keeps full ``torch.load`` compatibility instead of
assuming a tensor-only state dict.
"""
...
EMULATION_NOTE = (
    "Numerical emulation only: Conv2d/Linear weights are stored as integer buffers and dequantized "
    "for PyTorch floating-point F.conv2d/F.linear compute; activations use calibrated Q/DQ. "
    "This is not backend export and not accelerated NPU execution, so throughput is not a hardware speed claim."
)
...
class CalibrationStats:
    """Input activation statistics collected before an eligible Conv2d/Linear."""
    ...
    def update(self, module: nn.Module, x: Tensor) -> None:
        ...
        if x_detached.numel() == 0:
            return
        batch_min = float(x_detached.amin().cpu().item())
        batch_max = float(x_detached.amax().cpu().item())
        self.min_val = min(self.min_val, batch_min)
        self.max_val = max(self.max_val, batch_max)

        if isinstance(module, nn.Conv2d):
            if x_detached.ndim != 4:
                raise ValueError(f"{self.name}: Conv2d calibration input must be [B,C,H,W], got {tuple(x_detached.shape)}")
            if int(x_detached.shape[1]) != self.input_channels:
                raise ValueError(
                    f"{self.name}: expected {self.input_channels} input channels, got {int(x_detached.shape[1])}"
                )
            ...
            if x_detached.shape[-1] != self.input_channels:
                raise ValueError(
                    f"{self.name}: expected last dimension {self.input_channels}, got {int(x_detached.shape[-1])}"
                )
            flattened = x_detached.reshape(-1, self.input_channels)
        ...
        if int(per_channel_sum.numel()) != int(self.abs_sum.numel()):
            raise ValueError(
                f"{self.name}: calibration channel-stat shape changed from {self.abs_sum.numel()} "
                f"to {per_channel_sum.numel()}"
            )
        self.abs_sum += per_channel_sum
...
class EmulatedQuantizedConv2d(nn.Module):
    ...
    The integer weights are dequantized on every forward and passed to
    ``F.conv2d``. This preserves Conv2d numerics for evaluation proxies but is
    not a packed-kernel or accelerated quantized backend.
    """
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate Semantic-Guided CG-AF CNN checkpoints with FP32 and emulated actual "
            "quantized-weight/activation modes. Legacy Plan CA checkpoints remain compatible. "
            "This is PyTorch numerical emulation, not backend/NPU export."
        ),
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="NAME=PATH",
        help="Repeatable trusted project checkpoint spec, for example peft_cls_push=model/.../best_miou.pt.",
    )
    parser.add_argument("--modes", default=",".join(SUPPORTED_MODES), help=f"Comma-separated modes: {SUPPORTED_MODES}")
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=TABLES_DIR / "ca_semantic_actual_quant_eval",
        help="Directory for JSON metrics and combined CSV summary.",
    )
    parser.add_argument(
        "--summary-filename",
        default="ca_semantic_actual_quant_summary.csv",
        help="Combined CSV summary filename. Legacy default is preserved for old scripts.",
    )
    ...
    parser.add_argument("--max-eval-batches", type=int, default=None, help="Limit eval batches for smoke runs.")
    parser.add_argument("--seed", type=int, default=RANDOM_SEED)
    parser.add_argument("--no-validate-mask-values", action="store_false", dest="validate_mask_values")
    parser.add_argument(
        "--quantize-segmentation-head",
        action="store_true",
        help="Quantize segmentation_head; by default it follows QAT and is skipped.",
    )
    parser.add_argument(
        "--quantize-gates",
        action="store_true",
        help="Quantize gate_c2/gate_c3/gate_projection; by default they follow QAT and are skipped.",
    )
    parser.add_argument("--skip-pattern", action="append", default=[], help="Additional Conv/Linear name skip pattern.")
    ...
    parser.add_argument(
        "--self-test",
        action="store_true",
        help="Run wrapper unit-like smoke checks without loading checkpoints or datasets.",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.mask_source not in SEMANTIC_MASK_SOURCE_DEFAULT_MANIFESTS:
        raise ValueError(
            f"--mask-source must be one of {sorted(SEMANTIC_MASK_SOURCE_DEFAULT_MANIFESTS)}, got {args.mask_source!r}"
        )
...
def main() -> None:
    ...
    calibration_description = "not requested"
    if calibration_loader is not None:
        calibration_description = f"{args.calibration_split} ({len(calibration_loader.dataset)} images)"
    print(
        "Data: "
        f"mask_source={args.mask_source}, calibration={calibration_description}, eval_split={args.eval_split} "
        f"({len(eval_loader.dataset)} images), image_size={args.image_size}, device={device}",
        flush=True,
    )
    ...
    for checkpoint_spec in checkpoint_specs:
        checkpoint = load_checkpoint(checkpoint_spec.path)
        ...
        for mode in modes:
            ...
            print(
                f"{checkpoint_spec.name} {mode}: "
                f"acc={metrics['classification']['accuracy']:.4f} "
                f"macro_f1={metrics['classification']['macro_f1']:.4f} "
                f"mIoU={metrics['segmentation']['mean_iou']:.4f} "
                f"Dice={metrics['segmentation']['mean_dice']:.4f} "
                f"wrapped={quant_metadata['wrapped_count']} "
                f"json={json_path}",
                flush=True,
            )
            del model
...
def load_checkpoint(path: Path) -> Any:
    # Project checkpoints store metadata dicts/lists next to tensors.  Keep
    # weights_only=False for compatibility, and only use trusted project files.
...
def validate_checkpoint_ordering(
    ...
    *,
    ...
    """Validate class-index ordering metadata when a project checkpoint has it."""
    ...
    if metadata_has_content(class_to_idx):
        ...
        require_ordering_match(
            ...
            source="class_to_idx",
            actual=class_to_idx_order,
            expected=expected_scene_class_names,
        )
    ...
    if not scene_order_sources:
        warnings.append(
            "No class_to_idx/idx_to_class scene ordering metadata found; assuming dataset scene order "
            f"{expected_scene_class_names}."
        )
    ...
    if metadata_has_content(segmentation_classes):
        if not isinstance(segmentation_classes, (list, tuple)):
            raise ValueError(
                f"Checkpoint {checkpoint_spec.name} segmentation_classes metadata must be a list/tuple, "
                f"got {type(segmentation_classes).__name__}"
            )
        ...
        warnings.append(
            "No segmentation_classes ordering metadata found; assuming dataset segmentation order "
            f"{expected_segmentation_classes}."
        )
...
def ordering_from_class_to_idx(mapping: Any, *, source: str) -> list[str]:
    ...
    for class_name, raw_index in mapping.items():
        pairs.append((parse_metadata_index(raw_index, source=source), str(class_name)))
    return ordering_from_index_pairs(pairs, source=source)
...
def ordering_from_idx_to_class(mapping: Any, *, source: str) -> list[str]:
    if not isinstance(mapping, dict):
        raise ValueError(f"Checkpoint {source} metadata must be a dict, got {type(mapping).__name__}")
    pairs: list[tuple[int, str]] = []
    for raw_index, class_name in mapping.items():
        pairs.append((parse_metadata_index(raw_index, source=source), str(class_name)))
    return ordering_from_index_pairs(pairs, source=source)
...
def ordering_from_index_pairs(pairs: list[tuple[int, str]], *, source: str) -> list[str]:
    ...
    if sorted(indices) != expected_indices:
        raise ValueError(
            f"Checkpoint {source} class indices must be contiguous {expected_indices}, got {sorted(indices)}"
        )
...
def require_ordering_match(checkpoint_name: str, *, source: str, actual: list[str], expected: list[str]) -> None:
    if actual != expected:
        raise ValueError(
            ...
            f"actual={actual}, expected={expected}. Refusing to evaluate because class-index metrics would be ambiguous."
        )
...
def infer_model_config(
    ...
    *,
    ...
    segmentation_classes = (
        ...
        if isinstance(checkpoint_segmentation_classes, list) and checkpoint_segmentation_classes
    ...
    return ModelConfig(
        ...
        checkpoint_architecture=(
            str(checkpoint_dict["architecture"]) if checkpoint_dict.get("architecture") is not None else None
        ),
    )
...
def extract_state_dict(checkpoint: Any) -> dict[str, Tensor]:
    ...
    raise ValueError(
        "Checkpoint does not contain a recognizable state dict. Expected model_state_dict/state_dict/model or raw state_dict."
    )
    ...
    *,
    ...
    stats = {
        name: CalibrationStats(
            ...
            input_channels=input_channel_count(module),
        )
    ...
    try:
        ...
        for batch_index, (images, _masks, _scene_labels) in enumerate(progress, start=1):
            ...
            progress.set_postfix(images=observed_images)
    ...
    if unobserved:
        raise RuntimeError(
            "Calibration did not observe inputs for some quantized modules; refusing to guess activation ranges: "
            f"{unobserved[:20]}"
        )
...
def convert_model_to_emulated_quant(
    ...
    *,
        ...
        for child_name, child in list(parent.named_children()):
            ...
            if isinstance(child, (nn.Conv2d, nn.Linear)):
                ...
                if mode_spec.uses_awq:
                    awq_input_scale = compute_awq_input_scale(
                        stat,
                        alpha=awq_alpha,
                        scale_min=awq_scale_min,
                        scale_max=awq_scale_max,
                    )
    ...
    return {
        ...
        "weight_quantization": "per-output-channel symmetric signed integer weights stored as int8 buffers",
        "activation_qrange": [ACTIVATION_QMIN, ACTIVATION_QMAX],
        "activation_quantization": "per-module calibrated per-tensor affine uint8-range Q/DQ on module inputs",
...
def evaluate_model(
    ...
    *,
    ...
    for batch_index, (images, masks, scene_labels) in enumerate(progress, start=1):
        ...
        scene_confusion += batch_confusion(
            scene_predictions,
            scene_labels.cpu(),
            num_classes=len(scene_class_names),
            ignore_index=None,
        )
        segmentation_confusion += batch_confusion(
            segmentation_predictions,
            masks.cpu(),
            num_classes=len(segmentation_class_names_),
            ignore_index=SEMANTIC_IGNORE_INDEX,
        )
        observed_batches += 1
        observed_images += int(images.shape[0])
...
def activation_qparams(min_val: float, max_val: float, *, device: torch.device) -> tuple[Tensor, Tensor]:
    ...
    scale = ((max_tensor - min_tensor) / float(ACTIVATION_QMAX - ACTIVATION_QMIN)).clamp_min(
        torch.finfo(torch.float32).eps
    )
...
def compute_awq_input_scale(
    stat: CalibrationStats,
    *,
    alpha: float,
    scale_min: float,
    scale_max: float,
...
def conv_awq_weight_scale_view(module: nn.Conv2d, input_scale: Tensor, *, dtype: torch.dtype, device: torch.device) -> Tensor:
    if int(input_scale.numel()) != module.in_channels:
        raise ValueError(
            f"AWQ scale length {int(input_scale.numel())} does not match Conv2d in_channels={module.in_channels}"
        )
    if module.in_channels % module.groups != 0 or module.out_channels % module.groups != 0:
        raise ValueError(
            f"Conv2d groups must divide channels for AWQ mapping: in={module.in_channels}, "
            f"out={module.out_channels}, groups={module.groups}"
        )
...
def run_self_test() -> None:
    ...
    assert qconv.qweight.dtype == torch.int8
    ...
    if tuple(grouped_weight_view.shape) != expected_grouped_scale_shape:
        raise AssertionError(
            f"Grouped AWQ scale view shape {tuple(grouped_weight_view.shape)} != {expected_grouped_scale_shape}"
        )
    ...
    if int(qgrouped.awq_input_scale.numel()) != 4:
        raise AssertionError("Grouped Conv2d AWQ input scale should have one entry per global input channel")

    for depthwise_label, depthwise_out_channels in (("depthwise", 4), ("depthwise_multiplier", 8)):
        depthwise = nn.Conv2d(
            4,
            depthwise_out_channels,
            kernel_size=3,
            padding=1,
            groups=4,
            bias=False,
        ).eval()
        depthwise_x = torch.randn(2, 4, 8, 8)
        depthwise_stat = make_stats_from_input(depthwise_label, depthwise, depthwise_x)
        depthwise_scale = compute_awq_input_scale(depthwise_stat, alpha=0.5, scale_min=0.25, scale_max=4.0)
        depthwise_weight_view = conv_awq_weight_scale_view(
            depthwise,
            depthwise_scale,
            dtype=torch.float32,
            device=torch.device("cpu"),
        )
        expected_depthwise_scale_shape = (depthwise_out_channels, 1, 1, 1)
        if tuple(depthwise_weight_view.shape) != expected_depthwise_scale_shape:
            raise AssertionError(
                f"{depthwise_label} AWQ scale view shape {tuple(depthwise_weight_view.shape)} "
                f"!= {expected_depthwise_scale_shape}"
            )
        qdepthwise = EmulatedQuantizedConv2d(
            depthwise,
            name=depthwise_label,
            weight_bits=4,
            activation_min=depthwise_stat.min_val,
            activation_max=depthwise_stat.max_val,
            awq_input_scale=depthwise_scale,
        )
        assert_finite_shape(
            qdepthwise(depthwise_x),
            (2, depthwise_out_channels, 8, 8),
            f"awq {depthwise_label} conv",
        )
        if int(qdepthwise.awq_input_scale.numel()) != 4:
            raise AssertionError(f"{depthwise_label} AWQ input scale should have one entry per global input channel")
    ...
    if int(qlinear.awq_input_scale.numel()) != 6:
        raise AssertionError("Linear AWQ scale should have one entry per input feature")

    constant_stat = make_stats_from_input("constant", conv, torch.zeros(1, 3, 4, 4))
    qconstant = EmulatedQuantizedConv2d(
        conv,
        name="constant",
        weight_bits=8,
        activation_min=constant_stat.min_val,
        activation_max=constant_stat.max_val,
    )
    assert_finite_shape(qconstant(torch.zeros(1, 3, 4, 4)), (1, 5, 4, 4), "constant activation quant")

    print(
        "Self-test OK: Conv2d/Linear wrappers, grouped/depthwise AWQ mapping, "
        "non-zero padding, and activation Q/DQ are finite."
    )
