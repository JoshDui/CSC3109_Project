#!/usr/bin/env python3
"""Smoke tests for the Semantic-Guided CG-AF CNN."""
...
try:
    ...
    raise SystemExit("Semantic-Guided CG-AF smoke requires `torch`. Install project dependencies with `uv sync`.") from exc
...
from src.training.semantic_guided_losses import SemanticGuidedSceneLoss, SemanticGuidedSegmentationLoss


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Smoke-test Semantic-Guided CG-AF CNN forward/loss/backward on synthetic hard masks.",
        allow_abbrev=False,
    )
...
def main() -> None:
    ...
    run_smoke_case(
        ...
        backbone_name=SEMANTIC_GUIDED_CGAF_TEST_BACKBONE,
    ...
    if args.include_convnext_smoke:
        run_smoke_case(
            ...
            shallow_channels=shallow_channels,
            ignore_index=args.ignore_index,
            backbone_name=SEMANTIC_GUIDED_CGAF_CONVNEXT_TINY,
            qat_mode=args.qat_mode,
        )
...
def run_smoke_case(
    *,
    ...
    criterion = SemanticGuidedSegmentationLoss(ignore_index=ignore_index, include_background=True)
    scene_criterion = SemanticGuidedSceneLoss()
    ...
    assert_debug_outputs_valid(outputs, batch_size=batch_size)
    ...
    print(
        "semantic_guided_cgaf smoke OK "
        f"[{case_name}]: device={device}, backbone={backbone_name}, seg_shape={expected_segmentation_shape}, "
...
def assert_debug_outputs_valid(outputs: dict[str, torch.Tensor], *, batch_size: int) -> None:
...
def assert_clean_qat_state_loads(
    ...
    *,
    ...
    fresh_model = build_semantic_guided_cgaf_cnn(
        num_segmentation_classes=num_segmentation_classes,
        num_scene_classes=num_scene_classes,
        backbone_name=backbone_name,
        pretrained=False,
