...
def build_commands(args: argparse.Namespace, stages: list[str]) -> list[PipelineCommand]:
    ...
    if "split" in stages:
        commands.append(
            PipelineCommand(
                ...
                [
                    py,
                    str(args.project_root / "tools" / "semantic_guided" / "prepare_semantic_split_manifest.py"),
                    "--input",
    ...
    if "masks" in stages:
        commands.append(
            PipelineCommand(
                ...
                [
                    py,
                    str(args.project_root / "tools" / "semantic_guided" / "generate_sam3_class_aware_masks.py"),
                    "--manifest",
                    str(args.semantic_split_manifest),
                    "--config",
    ...
    if "loveda" in stages:
        commands.append(
            PipelineCommand(
                ...
                [
                    py,
                    str(args.project_root / "src" / "training" / "semantic_guided" / "train_loveda.py"),
                    "--data-root",
    ...
    if "quant" in stages:
        quant_command = [
            py,
            str(args.project_root / "tools" / "semantic_guided" / "evaluate_semantic_guided_quant.py"),
            "--run-id",
            str(args.run_id),
            "--checkpoint",
...
def dataset_commands(args: argparse.Namespace, py: str) -> list[PipelineCommand]:
    split_command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "check_semantic_dataset.py"),
        "--manifest",
        str(args.semantic_split_manifest),
        "--kind",
        "split",
...
def mask_export_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "export_semantic_guided_masks.py"),
        "--run-id",
        str(args.run_id),
        "--checkpoint",
        f"fft={args.fft_checkpoint}",
...
def onnx_export_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "export_semantic_guided_onnx.py"),
        "--run-id",
        str(args.run_id),
        "--checkpoint",
        f"fft={args.fft_checkpoint}",
...
def onnx_eval_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "evaluate_semantic_guided_onnx.py"),
        "--run-id",
        str(args.run_id),
        "--checkpoint",
        f"fft={args.fft_checkpoint}",
...
def onnx_delivery_size_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    return PipelineCommand(
        ...
        [
            py,
            str(args.project_root / "tools" / "semantic_guided" / "summarize_artifact_delivery_sizes.py"),
            "--run-id",
            str(args.run_id),
            "--artifact",
...
def unseen_val12_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "evaluate_semantic_guided_unseen_imagefolder.py"),
        "--run-id",
        str(args.run_id),
        "--data-dir",
...
def unseen_review_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    ...
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "create_semantic_guided_unseen_review_artifacts.py"),
        "--table-dir",
        str(args.unseen_output_dir),
        "--mask-dir",
        str(args.unseen_mask_dir),
...
def onnx_case_study_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "run_semantic_guided_onnx_case_study.py"),
        "--run-id",
        str(args.run_id),
        "--onnx-fp32-path",
        str(args.onnx_fp32_path),
...
def awq_onnx_case_study_command(args: argparse.Namespace, py: str) -> PipelineCommand:
    command = [
        py,
        str(args.project_root / "tools" / "semantic_guided" / "compare_semantic_guided_case_studies.py"),
        "--run-id",
        str(args.run_id),
...
def transfer_command(args: argparse.Namespace, py: str, *, mode: str, output_dir: Path) -> PipelineCommand:
    return PipelineCommand(
        ...
        [
            py,
            str(args.project_root / "src" / "training" / "semantic_guided" / "train_transfer.py"),
            "--fine-tuning-mode",
            mode,
            "--manifest-path",
            str(args.sam3_mask_manifest),
