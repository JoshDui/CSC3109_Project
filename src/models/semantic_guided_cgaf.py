"""Semantic-Guided CG-AF CNN model implementation.

The module is self-contained so final training, evaluation, and smoke tests do
not depend on removed experiment modules.  Internal module attribute names are
kept stable for state-dict-only checkpoint loading.
"""
...
SEMANTIC_GUIDED_CGAF_CONVNEXT_TINY = CONVNEXT_FEATURE_BACKBONE_TINY
SEMANTIC_GUIDED_CGAF_TEST_BACKBONE = CONVNEXT_FEATURE_TEST_BACKBONE


class PointwiseConvNormAct(nn.Sequential):
    """1x1 Conv-Norm-Activation block."""
...
class SpatialContextGate(nn.Module):
    """Spatial sigmoid gate for a shallow feature using low-resolution context."""
...
class SegmentationGuidedSceneHead(nn.Module):
    """Scene classifier whose input is explicitly conditioned on segmentation logits."""
...
class SemanticGuidedCGAFCNN(nn.Module):
    """Context-gated asymmetric-fusion CNN for semantic masks and scene labels."""
    ...
    def __init__(
        ...
        *,
        ...
        if len(feature_channels) != 4:
            raise ValueError(f"Semantic-Guided CG-AF CNN expects four feature levels C2-C5, got {len(feature_channels)}")
        if num_segmentation_classes <= 0:
            raise ValueError(f"num_segmentation_classes must be positive, got {num_segmentation_classes}")
        if fpn_channels <= 0:
            raise ValueError(f"fpn_channels must be positive, got {fpn_channels}")
    ...
    def forward(self, images: Tensor, *, return_scene: bool = False, return_debug: bool = False) -> dict[str, Tensor]:
        ...
        if return_scene and not self.enable_scene_head:
            raise RuntimeError(
                "return_scene=True was requested, but this Semantic-Guided CG-AF CNN was built "
                "with enable_scene_head=False"
            )
        ...
        if isinstance(features, Tensor) or len(features) != 4:
            raise RuntimeError(
                "Semantic-Guided CG-AF CNN backbone must return a sequence of four feature maps: C2, C3, C4, C5"
            )
        ...
        if return_scene:
            if self.scene_head is None:
                raise RuntimeError("Semantic-guided scene head modules are not available")
            outputs.update(
                self.scene_head(
                    decoder_features,
                    low_resolution_logits,
...
def build_semantic_guided_cgaf_cnn(
    *,
    num_segmentation_classes: int = 5,
    num_scene_classes: int = 4,
    ...
    """Build the Semantic-Guided CG-AF CNN.

    Use ``backbone_name=SEMANTIC_GUIDED_CGAF_TEST_BACKBONE`` for fast local
    smoke tests.  The default path constructs a timm ConvNeXt-Tiny feature
    extractor with ``features_only=True``.
    """
...
def _build_backbone(
    *,
    ...
    if backbone_name == SEMANTIC_GUIDED_CGAF_TEST_BACKBONE:
    ...
    try:
        ...
        raise ImportError(
            "Semantic-Guided CG-AF CNN requires `timm` for production backbones. "
            "Install project dependencies with `uv sync`, or use "
            f"backbone_name={SEMANTIC_GUIDED_CGAF_TEST_BACKBONE!r} for smoke tests."
        ) from exc
    ...
    if len(feature_channels) != 4:
        raise RuntimeError(
            "Semantic-Guided CG-AF CNN requires four timm feature maps for C2-C5; "
