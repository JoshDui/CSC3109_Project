"""Transfer-learn the Semantic-Guided CG-AF CNN with SAM3 pseudo-masks.

SAM3 is used only as an offline teacher: this trainer consumes the project
classification images plus generated semantic pseudo-mask PNGs through
``create_semantic_dataloaders(mask_source="sam3_class_aware")``.  No SAM3
library/model is required at train or inference time.
"""
...
from src.data.semantic_segmentation import SEMANTIC_IGNORE_INDEX
from src.data.semantic_segmentation import SEMANTIC_MASK_SOURCE_VALUES, SEMANTIC_MASK_SOURCE_DEFAULT_MANIFESTS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Transfer-learn the Semantic-Guided CG-AF CNN with segmentation-guided scene "
            "classification on SAM3 pseudo-masks. Legacy Plan CA checkpoints remain compatible."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Semantic mask source. Defaults to SAM3 class-aware raw pseudo-masks.",
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.25,
        help="Backbone LR multiplier for transfer learning differential LR.",
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Optional LoveDA Semantic-Guided CG-AF CNN/legacy Plan CA checkpoint to partially load by compatible key/shape.",
    )
    parser.add_argument(
        "--pretrained",
        action="store_true",
    ...
    parser.add_argument(
        ...
        action="store_false",
        dest="pretrained",
        help="Do not initialize the backbone from timm pretrained weights.",
    )
    ...
    parser.add_argument(
        ...
        choices=("none", "cosine"),
        default="cosine",
        help="Learning-rate schedule. 'none' disables scheduling and ignores warmup/min-lr.",
    )
    parser.add_argument("--warmup-epochs", type=int, default=1, help="Cosine warmup epochs; coerced to 0 with --scheduler none.")
    parser.add_argument("--min-lr", type=float, default=0.0, help="Cosine minimum LR; ignored with --scheduler none.")
    parser.add_argument(
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if args.scene_weight < 0.0:
        raise ValueError(f"--scene-weight must be non-negative, got {args.scene_weight}")
    if args.segmentation_weight < 0.0:
        raise ValueError(f"--segmentation-weight must be non-negative, got {args.segmentation_weight}")
    if args.ce_weight < 0.0:
        raise ValueError(f"--ce-weight must be non-negative, got {args.ce_weight}")
    if args.dice_weight < 0.0:
        raise ValueError(f"--dice-weight must be non-negative, got {args.dice_weight}")
...
def resolve_pretrained_setting(args: argparse.Namespace) -> None:
    ...
    if args.checkpoint is not None:
        ...
        print(
            "Checkpoint provided; building Semantic-Guided CG-AF CNN with pretrained=False before partial checkpoint load. "
            "Pass --pretrained to also initialize timm pretrained weights first.",
            flush=True,
        )
    else:
...
def default_output_dir(mask_source: str) -> Path:
    return MODEL_DIR / f"plan_ca_semantic_{slugify(mask_source)}"
...
def load_partial_checkpoint(model: nn.Module, checkpoint_path: Path) -> dict[str, Any]:
    ...
    for raw_key, value in source_state.items():
        ...
        if tuple(value.shape) != tuple(current_state[key].shape):
            skipped.append(
                {
                    "key": key,
                    "reason": f"shape {tuple(value.shape)} != target {tuple(current_state[key].shape)}",
                }
            )
            continue
    ...
    if skipped:
        formatted = "; ".join(f"{item['key']} ({item['reason']})" for item in skipped[:8])
        print(f"  skipped examples: {formatted}", flush=True)
    return info
...
def extract_state_dict(checkpoint: Any) -> dict[str, Any]:
    if isinstance(checkpoint, dict):
        ...
        if looks_like_state_dict(checkpoint):
            return checkpoint
    raise ValueError(
        "Checkpoint does not contain a recognizable state dict. Expected a dict with model_state_dict/state_dict/model "
...
def audit_semantic_manifest(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.audit_hash_images:
        ...
        if duplicate_hashes:
            examples = []
            for digest in duplicate_hashes[:10]:
                train_paths = [path for path, value in hashes_by_split[args.train_split].items() if value == digest]
                tune_paths = [path for path, value in hashes_by_split[args.tune_split].items() if value == digest]
                examples.append({"sha256": digest, "train": train_paths[:3], "tune": tune_paths[:3]})
            raise ValueError(f"Manifest train/tune duplicate image content detected: {examples}")
    summary = {
...
def evaluate(
    ...
    *,
    ...
    segmentation_confusion = torch.zeros(
        (len(segmentation_class_names_), len(segmentation_class_names_)), dtype=torch.int64
    )
...
def segmentation_metrics_from_confusion(confusion: Tensor, class_names: list[str]) -> dict[str, Any]:
    ...
    return {
        ...
        "per_class_iou": {
            name: float(value.item()) if not torch.isnan(value) else 0.0 for name, value in zip(class_names, iou)
        },
        "per_class_dice": {
            name: float(value.item()) if not torch.isnan(value) else 0.0 for name, value in zip(class_names, dice)
        },
        "confusion_matrix": confusion.tolist(),
    }
    ...
    torch.save(
        {
            ...
            "architecture": getattr(args, "checkpoint_architecture", "plan_ca_semantic_guided"),
...
def main() -> None:
    ...
    if args.output_dir is None:
        args.output_dir = default_output_dir(args.mask_source)
    set_seed(args.seed)
    ...
    if qat_config.mode != "none":
        ...
        print(f"QAT prepared: mode={qat_config.mode}, wrapped={qat_prepare.wrapped_count}, skipped={len(qat_prepare.skipped_names)}", flush=True)
    ...
    print(
        ...
        f"device={device}, amp={use_amp}, mask_source={args.mask_source}, "
