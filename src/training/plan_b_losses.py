"""Loss utilities for Plan B hard-mask semantic training."""
...
def multiclass_dice_loss(
    ...
    *,
    ...
    ``targets`` must contain integer class IDs in ``[0, C)`` and may contain
    ``ignore_index``. Ignored pixels are removed from both predictions and
    one-hot targets before Dice statistics are accumulated.
    """
    ...
    valid_mask = valid_mask.unsqueeze(1).to(dtype=logits.dtype)
...
class PlanBSegmentationLoss(nn.Module):
...
class PlanBSceneLoss(nn.Module):
...
class PlanBJointLoss(nn.Module):
    """Joint Plan B loss: scene CE plus weighted hard-mask segmentation loss."""
    ...
    def __init__(
        ...
        *,
        ...
        self.segmentation_loss = PlanBSegmentationLoss(
        ...
        self.scene_loss = PlanBSceneLoss(label_smoothing=label_smoothing)
    ...
    def forward(
        ...
        if "segmentation_logits" not in outputs:
            raise KeyError("PlanBJointLoss requires outputs['segmentation_logits']")
        if "scene_logits" not in outputs:
            raise KeyError("PlanBJointLoss requires outputs['scene_logits']")
...
def segmentation_ce_or_focal_loss_map(
    ...
    *,
    ...
    """Return per-pixel CE or focal CE map for hard segmentation targets."""
