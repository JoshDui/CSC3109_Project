...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--checkpoint", type=Path, default=PROJECT_ROOT / "model/hetmcl_lite/best_model.pt")
    ...
    parser.add_argument("--output-dir", type=Path, default=REPORTS_DIR / "hetmcl_reliability")
...
def missing_records_for_split(manifest: Path, split: str) -> list[str]:
    ...
    for record in load_manifest_records(manifest, split):
        if not record.image_path.exists():
            missing.append(str(record.image_path))
    return missing
...
def main() -> None:
    ...
    if plot_learning_curves(args.history, args.output_dir / "learning_curves.png"):
        summary["learning_curves"] = str(args.output_dir / "learning_curves.png")
    else:
